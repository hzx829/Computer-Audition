...
class ModelSingleStep(torch.nn.Module):
    ...
    def decode(self, h,h_pre,h_delay):
        
        r_output = self.grucell(h,h_pre)
        m1= torch.cat((h_delay,r_output),1)
    ...
    def forward(self, x,h_pre,h_delay):
        #glue the encoder and the decoder together
        h = self.encode(x)
        o,r_output = self.decode(h,h_pre,h_delay)
        return o,r_output
...
if __name__ == "__main__":
    ...
    savedFilename = "ModelRNN_delay.pt"
            ...
            for i in  range(seqLen):
                ...
                if i <5:#delay is 5
                    ...
                    if i==0:
                        ...
                        GRUout.append(GRUo)
                        output.append(outp)
                    outp,GURo = model.forward(mixture[i,:,:],GRUout[i-1],torch.zeros_like(h))
                    GRUout.append(GRUo)
                    output.append(outp)
                else:
